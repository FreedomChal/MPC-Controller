    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          px += v * cos(psi) * delay;
          py += v * sin(psi) * delay;
          psi += v * -steering_angle * delay / Lf;

          std::vector<double> vehicle_coord_x;
          std::vector<double> vehicle_coord_y;

          for(int i = 0; i < ptsx.size(); i++) {
            double dx = ptsx[i] - px;
            double dy = ptsy[i] - py;
            double new_x = (dx * cos(psi)) - (dy * sin(-psi));
            double new_y = (dx * sin(-psi)) + (dy * cos(psi));
            vehicle_coord_x.push_back(new_x);
            vehicle_coord_y.push_back(new_y);
          }
